...
public class P2 {
	...
	private static String hostname = "P2";
	...
	private LinkedList<ActivePeerDetailsForP2> activePeersDatabase = new LinkedList<ActivePeerDetailsForP2>();
	...
	private static final String RECEIVE_ON_FILE_PATH = System.getProperty("user.dir") + "\\RFCP2";
...
private class ActivePeerDetailsForP2 {
		...
		ActivePeerDetailsForP2(String receivedHostname, long receivedPortNumberOfRFCServerOfActivePeer){
...
private class PeriodicTasks extends TimerTask {
	...
	public void run() {
		...
		try {
			...
			int index = 0;
			for(RFCIndex rfcIndex : rfcDatabase) {
				if(!rfcIndex.getHostnameOfPeerContainingRFC().equals("P2")) {
	...
	public void registerCycle() throws IOException {
		...
		InputStream inputStream = null;
				try {
					...
					portNumberOfRFCServerOfPeer = 65402;
					...
					System.out.println("RFC server port of P2 is " + portNumberOfRFCServerOfPeer + "\n");
				...
				try {
					...
					while(scanner.hasNext()) {
						if(counter == 0 && numberOfActivePeersReceived > 0) {
							...
							activePeersDatabase.add(new ActivePeerDetailsForP2(receivedHostname, receivedPortNumberOfRFCServerOfActivePeer));
							counter = 0;
						}
					}
					...
					for(ActivePeerDetailsForP2 activePeerDetailsForP2 : activePeersDatabase) {
						System.out.println(activePeerDetailsForP2.getReceivedHostname());
						System.out.println(activePeerDetailsForP2.getReceivedPortNumberOfRFCServerOfActivePeer());
					}
		...
		if(activePeersDatabase.size() != 0) {
			for(ActivePeerDetailsForP2 activePeerDetailsForP2 : activePeersDatabase) {
				try {
					 ...
					 */
					clientSocket = new Socket("127.0.0.1", (int) activePeerDetailsForP2.getReceivedPortNumberOfRFCServerOfActivePeer());
					outputStream = new DataOutputStream(clientSocket.getOutputStream());
					System.out.println("GET RFC-Index P2P-DI/1.0");
					System.out.println("Host: " + activePeerDetailsForP2.getReceivedHostname());
					System.out.println("OS: " + System.getProperty("os.name") + "\n");
	...
	public void retrieveRFCServerPortsOfActivePeers() {
		for (ActivePeerDetailsForP2 activePeerDetailsForP2 : activePeersDatabase) {
			if(activePeerDetailsForP2.getReceivedHostname().equals("P0")) {
				rfcServerPortOfP0 = activePeerDetailsForP2.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP2.getReceivedHostname().equals("P1")) {
				rfcServerPortOfP1 = activePeerDetailsForP2.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP2.getReceivedHostname().equals("P3")) {
				rfcServerPortOfP3 = activePeerDetailsForP2.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP2.getReceivedHostname().equals("P4")) {
				rfcServerPortOfP4 = activePeerDetailsForP2.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP2.getReceivedHostname().equals("P5")) {
				rfcServerPortOfP5 = activePeerDetailsForP2.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
		}
	}
...
public void getRFCFileFromOtherPeers() throws UnknownHostException, IOException {
	 ...
	 */
	FileWriter fileWriter = new FileWriter("CumulativeTimeForP2");
...
public void addRFCInIndexForP2P() {
	rfcDatabase.add(new RFCIndex(8242, "Interface_to_the_Routing_System_(I2RS)_Ephemeral_State_Requirements", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8241, "Interface_to_the_Routing_System_(I2RS)_Security-Related_Requirements", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8240, "Report_from_the_Internet_of_Things_Software_Update_(IoTSU)_Workshop_2016", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8239, "Data_Center_Benchmarking_Methodology", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8238, "Data_Center_Benchmarking_Terminology", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8236, "J-PAKE:_Password-Authenticated_Key_Exchange_by_Juggling", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8235, "Schnorr_Non-interactive_Zero-Knowledge_Proof", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8234, "Updates_to_MPLS_Transport_Profile_(MPLS-TP)_Linear_Protection_in_Automatic_Protection_Switching_(APS)_Mode", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8233, "Extensions_to_the_Path_Computation_Element_Communication_Protocol_(PCEP)_to_Compute_Service-Aware_Label_Switched_Paths_(LSPs)", "P2", 7200));
	rfcDatabase.add(new RFCIndex(8232, "Optimizations_of_Label_Switched_Path_State_Synchronization_Procedures_for_a_Stateful_PCE", "P2", 7200));
}
...
public void leaveCycle() throws IOException {
	...
	public static void main(String[] args) throws IOException {
		...
		p2.registration = reader.next();
		if(p2.registration.equalsIgnoreCase("y")) {
			p2.registerCycle();
		}
		/*
		...
		Runnable r1 = new Runnable() {
			...
			@Override
			public void run() {
				ServerSocket rfcServerSocketForP2 = null;
				try {
					rfcServerSocketForP2 = new ServerSocket(65402);
					ExecutorService executorService = Executors.newCachedThreadPool();
					while(true) {
						Socket connectionSocket = rfcServerSocketForP2.accept();
						Runnable r2 = p2.new ThreadToSendRFCIndexOrRFCToPeer(connectionSocket);
		...
		p2.task = reader.nextInt();
		switch(p2.task) {
		case 1: break;
		case 2: p2.addRFCInIndexForP2P();
				break;
		}
		
		System.out.println("Please press 'y' to get list of active peers from Registration Server");
		p2.pQuery = reader.next();
		if(p2.pQuery.equalsIgnoreCase("y")) {
			p2.getActivePeersFromRS();
		}
		
		System.out.println("Please press 'y' to get RFC index from all other active peers");
		p2.rfcIndexQuery = reader.next();
		if(p2.rfcIndexQuery.equalsIgnoreCase("y")) {
			p2.getRFCIndexFromOtherPeers();
		}
		
		System.out.println("Please press 'y' to download all the RFC files");
		p2.rfcFileQuery = reader.next();
		if(p2.rfcFileQuery.equalsIgnoreCase("y")) {
			p2.getRFCFileFromOtherPeers();
		}
		
		
		Timer time = new Timer();
		PeriodicTasks periodicTasks = p2.new PeriodicTasks();
