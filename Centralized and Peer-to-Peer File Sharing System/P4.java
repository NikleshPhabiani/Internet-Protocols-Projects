...
public class P4 {
	private static String hostname = "P4";
	...
	private LinkedList<ActivePeerDetailsForP4> activePeersDatabase = new LinkedList<ActivePeerDetailsForP4>();
	...
	private static final String RECEIVE_ON_FILE_PATH = System.getProperty("user.dir") + "\\RFCP4";
...
private class ActivePeerDetailsForP4 {
		...
		ActivePeerDetailsForP4(String receivedHostname, long receivedPortNumberOfRFCServerOfActivePeer){
...
private class PeriodicTasks extends TimerTask {
	...
	public void run() {
		...
		try {
			...
			int index = 0;
			for(RFCIndex rfcIndex : rfcDatabase) {
				if(!rfcIndex.getHostnameOfPeerContainingRFC().equals("P4")) {
	...
	public void registerCycle() throws IOException {
		...
		InputStream inputStream = null;
				try {
					...
					portNumberOfRFCServerOfPeer = 65404;
					...
					System.out.println("RFC server port of P4 is " + portNumberOfRFCServerOfPeer + "\n");
				...
				try {
					...
					while(scanner.hasNext()) {
						if(counter == 0 && numberOfActivePeersReceived > 0) {
							...
							activePeersDatabase.add(new ActivePeerDetailsForP4(receivedHostname, receivedPortNumberOfRFCServerOfActivePeer));
							counter = 0;
						}
					}
					...
					for(ActivePeerDetailsForP4 activePeerDetailsForP4 : activePeersDatabase) {
						System.out.println(activePeerDetailsForP4.getReceivedHostname());
						System.out.println(activePeerDetailsForP4.getReceivedPortNumberOfRFCServerOfActivePeer());
					}
		...
		if(activePeersDatabase.size() != 0) {
			for(ActivePeerDetailsForP4 activePeerDetailsForP4 : activePeersDatabase) {
				try {
					 ...
					 */
					clientSocket = new Socket("127.0.0.1", (int) activePeerDetailsForP4.getReceivedPortNumberOfRFCServerOfActivePeer());
					outputStream = new DataOutputStream(clientSocket.getOutputStream());
					System.out.println("GET RFC-Index P2P-DI/1.0");
					System.out.println("Host: " + activePeerDetailsForP4.getReceivedHostname());
					System.out.println("OS: " + System.getProperty("os.name") + "\n");
	...
	public void retrieveRFCServerPortsOfActivePeers() {
		for (ActivePeerDetailsForP4 activePeerDetailsForP4 : activePeersDatabase) {
			if(activePeerDetailsForP4.getReceivedHostname().equals("P0")) {
				rfcServerPortOfP0 = activePeerDetailsForP4.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP4.getReceivedHostname().equals("P1")) {
				rfcServerPortOfP1 = activePeerDetailsForP4.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP4.getReceivedHostname().equals("P2")) {
				rfcServerPortOfP2 = activePeerDetailsForP4.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP4.getReceivedHostname().equals("P3")) {
				rfcServerPortOfP3 = activePeerDetailsForP4.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
			if(activePeerDetailsForP4.getReceivedHostname().equals("P5")) {
				rfcServerPortOfP5 = activePeerDetailsForP4.getReceivedPortNumberOfRFCServerOfActivePeer();
			}
		}
	}
...
public void getRFCFileFromOtherPeers() throws UnknownHostException, IOException {
	 ...
	 */
	FileWriter fileWriter = new FileWriter("CumulativeTimeForP4");
...
public void addRFCInIndexForP2P() {
	rfcDatabase.add(new RFCIndex(8218, "Multipath_Extension_for_the_Optimized_Link_State_Routing_Protocol_Version_2_(OLSRv2)", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8217, "Clarifications_for_When_to_Use_the_name-addr_Production_in_SIP_Messages", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8216, "HTTP_Live_Streaming", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8215, "Local-Use_IPv4/IPv6_Translation_Prefix", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8214, "Virtual_Private_Wire_Service_Support_in_Ethernet_VPN", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8213, "Security_of_Messages_Exchanged_between_Servers_and_Relay_Agents", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8212, "Default_External_BGP_(EBGP)_Route_Propagation_Behavior_without_Policies", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8211, "Adverse_Actions_by_a_Certification_Authority_(CA)_or_Repository_Manager_in_the_Resource_Public_Key_Infrastructure_(RPKI).", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8210, "The_Resource_Public_Key_Infrastructure_(RPKI)_to_Router_Protocol_Version_1", "P4", 7200));
	rfcDatabase.add(new RFCIndex(8209, "A_Profile_for_BGPsec_Router_Certificates,_Certificate_Revocation_Lists,_and_Certification_Requests", "P4", 7200));
}
...
public void leaveCycle() throws IOException {
	...
	public static void main(String[] args) throws IOException {
		...
		p4.registration = reader.next();
		if(p4.registration.equalsIgnoreCase("y")) {
			p4.registerCycle();
		}
		/*
		...
		Runnable r1 = new Runnable() {
			...
			@Override
			public void run() {
				ServerSocket rfcServerSocketForP4 = null;
				try {
					rfcServerSocketForP4 = new ServerSocket(65404);
					ExecutorService executorService = Executors.newCachedThreadPool();
					while(true) {
						Socket connectionSocket = rfcServerSocketForP4.accept();
						Runnable r2 = p4.new ThreadToSendRFCIndexOrRFCToPeer(connectionSocket);
		...
		p4.task = reader.nextInt();
		switch(p4.task) {
		case 1: break;
		case 2: p4.addRFCInIndexForP2P();
				break;
		}
		
		System.out.println("Please press 'y' to get list of active peers from Registration Server");
		p4.pQuery = reader.next();
		if(p4.pQuery.equalsIgnoreCase("y")) {
			p4.getActivePeersFromRS();
		}
		
		System.out.println("Please press 'y' to get RFC index from all other active peers");
		p4.rfcIndexQuery = reader.next();
		if(p4.rfcIndexQuery.equalsIgnoreCase("y")) {
			p4.getRFCIndexFromOtherPeers();
		}
		
		System.out.println("Please press 'y' to download all the RFC files");
		p4.rfcFileQuery = reader.next();
		if(p4.rfcFileQuery.equalsIgnoreCase("y")) {
			p4.getRFCFileFromOtherPeers();
		}
		
		
		Timer time = new Timer();
		PeriodicTasks periodicTasks = p4.new PeriodicTasks();
